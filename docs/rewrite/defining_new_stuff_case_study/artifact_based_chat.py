...
class MessageWithDiff(Component[list[str | JSONDiff]]):
    ...
    def format_for_llm(self):
        return TemplateRepresentation(
            obj=self,
            role=self.role,
            args={"diffs": self._diffs, "content": self.content},
        )

    def _parse(self, computed: ModelOutputThunk[Any]) -> "MessageWithDiff":
        value: str = computed.value
        assert value is not None, (
            "library error: should never call parse() on an uncomputed mot"
        )
                ...
                case "diff":
                    ...
                    if value[curr_idx : curr_idx + 3] == "```":
                        try:
                            ...
                            raise TypeError(
                                f"Expected diff {curr_part} to be valid JSON but got error during parsing: {e}"
                            )
        ...
        assert all([type(p) != str for p in diff_parts]), (
            f"Found strings in diff_parts: {diff_parts}; text_parts was: {text_parts} with diff types: {[type(part) for part in diff_parts]}"
        )
        return MessageWithDiff(
            role="assistant", content="".join(text_parts), diffs=diff_parts
        )
...
class ArtifactChatContext(Context):
    ...
    def __init__(
        self,
        chat_history: list[MessageWithDiff | ModelOutputThunk],
        origin_artifact: dict | list,
    ):
        self._chat_history = chat_history
        self._origin_artifact = origin_artifact
        self._system_msg = Message(
            role="system",
            content='You are an assistant whose job is to make changes to a JSON object. If the user asks a question about the JSON, answer their question. If the user asks you to make changes to the JSON, reply with a JSON object containing keys and their new values. For example, if the user says \'change the value of x to 5 and change th value of y to 2\', then you should reply with ```json\n{"x": 5,\n"y": 2}\n```',
        )
    ...
    def update_artifact(self, artifact: dict | list, diff: JSONDiff) -> dict | list:
        ...
        match diff:
            ...
            case _:
                raise Exception(
                    f"Diff type not supported: {type(diff)} for diff {diff}"
                )
        return new_artifact

    def _process_msg_diffs(
        self,
        artifact: list | dict,
        msg: MessageWithDiff | ModelOutputThunk[MessageWithDiff],
    ) -> list | dict:
        match msg:
            ...
            case ModelOutputThunk():
                assert msg.is_computed(), (
                    "Object Protocol Error in the Artifact-based chat library: must wait for all actions in the context to be computed before asking for the current state of the artifact."
                )
                assert msg.parsed_repr is not None, (
                    "Core error: his assert should NEVER fail because we already asserted is_computed"
                )
                return self._process_msg_diffs(artifact, msg.parsed_repr)
            case _:
                raise TypeError(
                    f"Expected MessageWithDiff or ModelOutputThunk[MessageWithDiff] but found {type(msg)}"
                )
    ...
    def add(self, mot: ModelOutputThunk[MessageWithDiff]) -> "ArtifactChatContext":
        ...
        return ArtifactChatContext(
            chat_history=new_history, origin_artifact=self._origin_artifact
        )
...
async def main():
    ...
    mot, new_ctx = await backend.generate_from_context(
        MessageWithDiff(role="user", content="Set x to 2 in the dictionary.", diffs=[]),
        ctx=ctx,
    )
    await mot.avalue()
