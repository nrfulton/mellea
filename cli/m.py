...
(start a model-serving endpoint), `m mitm` (proxy an OpenAI-compatible endpoint and
intercept requests), `m alora` (train and upload LoRA/aLoRA adapters), `m decompose`
(LLM-driven task decomposition), and `m eval` (test-based model evaluation). Run
...
from cli.mitm.commands import mitm
...
def callback() -> None:
    ...
    Provides sub-commands for serving models (`m serve`), proxying and intercepting
    an OpenAI-compatible endpoint (`m mitm`), training and uploading adapters
    (`m alora`), decomposing tasks into subtasks (`m decompose`), running test-based
    evaluation pipelines (`m eval`), and applying automated code migrations
    (`m fix`).
