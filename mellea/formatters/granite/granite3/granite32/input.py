...
# First Party
from ...base.types import ChatCompletion
from ...granite3.constants import (
...
class Granite32InputProcessor(Granite3InputProcessor):
    r"""Input processor for version 3.2 of the main Granite models, all sizes.
    ...
    def transform(
        ...
        if not tools:
            ...
            tools_part = (
                "<|start_of_role|>tools<|end_of_role|>"
                + json.dumps([t.model_dump() for t in tools], indent=4)
                + "<|end_of_text|>\n"
            )
        ...
        if not docs:
            ...
            documents_body = "\n\n".join(
                [f"Document {i}\n{docs[i].text}" for i in range(len(docs))]
            )
            documents_part = (
                "<|start_of_role|>documents<|end_of_role|>"
                + documents_body
